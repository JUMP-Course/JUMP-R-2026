...
tbl_final <- tbl_base %>%
  ...
  modify_caption("表2. 研究人群加权基线特征及组间比较") %>%
  bold_labels()
...
knitr::kable(logistic_results, caption = "表3.尿镉与桥本甲状腺炎的多因素Logistic回归")
```
...
knitr::kable(dose_response_table, caption = "表4.尿镉与桥本甲状腺炎的剂量-反应分析")
```
...
knitr::kable(subgroup_results, caption = "表5. 亚组分层分析结果")
```
...
knitr::kable(interaction_results, caption = "表6. 交互作用检验结果")
```
...
1. **基线特征**：桥本阳性组与阴性组相比，年龄更大、尿镉水平更高、女性比例更高（均P<0.001）；两组在教育程度（P=0.044）和贫困收入比（P=0.010）方面也有显著差异；BMI、吸烟状况和高血压在两组间无显著差异（P>0.05）。
