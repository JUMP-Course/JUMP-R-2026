...
output:
  html_document:
    toc: TRUE               # 开启目录
    toc_float: TRUE         # 目录悬浮侧边栏（浏览更方便）
    toc_depth: 3            # 目录抓取到3级标题（##二级、###三级）
    number_sections: TRUE   # 自动给标题加1、1.1、1.2编号
date: "2026-06-08"
---
...
# 5.1 读取并合并原始数据
demo   <- read_xpt("doc/DEMO_J.XPT")
bmx    <- read_xpt("doc/BMX_J.XPT")
dr1tot <- read_xpt("doc/DR1TOT_J.XPT")
tc_dat    <- read_xpt("doc/TCHOL_J.XPT")
tgldl_dat <- read_xpt("doc/TRIGLY_J.XPT")
hdl_dat   <- read_xpt("doc/HDL_J.XPT")
...
# 生成膳食添加糖三分组分层基线表与组间差异检验
tab_group <- CreateTableOne(
  vars = c(con_vars, cat_vars),       
  strata = "exposure_3", # 分层变量：膳食添加糖三分组
  data = df_final,
  factorVars = cat_vars
)
print(tab_group, 
...
tab_total_export <- print(tab_total,
                          ...
                          pDigits = 3,
                          printToggle = FALSE)
write.csv(tab_total_export, file="总体描述统计表.csv", row.names = TRUE)
...
tab_group_export <- print(tab_group,
                          ...
                          test = TRUE,
                          printToggle = FALSE)
write.csv(tab_group_export, file="分组描述统计表.csv", row.names = TRUE)
```
...
df_trend_hdl <- summarise(
  df_group,
  mean_hdl = mean(LBXHD, na.rm = TRUE),
  se_hdl = sd(LBXHD, na.rm = TRUE)/sqrt(n()),
  lower = mean_hdl - 1.96*se_hdl,
  upper = mean_hdl + 1.96*se_hdl
)
...
# 包安装统一放置文档setup块，本段仅执行加载，避免重复安装
library(dplyr)
library(dunn.test)
library(car)
library(stargazer)
...
### 8.2 单变量组间差异检验
## 8.2.1 连续血脂指标多组比较：Kruskal-Wallis检验+Dunn事后两两比较（Bonferroni校正）
for(var in lipid_con){
  cat("\n血脂指标：", var, "\n")
  dunn.test(x = df_final[[var]], g = df_final$exposure_3, method = "bonferroni")
}

## 8.2.2 分类血脂异常患病构成：Pearson卡方检验
...
## 8.2.3 膳食添加糖与血脂Spearman秩相关分析（适配偏态膳食数据）
for(var in lipid_con){
...
## 8.3.2 连续添加糖暴露Logistic回归（二分类血脂异常，输出OR值）
mod_store <- list() # 存储全部M3完整校正模型，用于后续模型诊断
...
## 8.3.3 三分组分类添加糖Logistic回归（验证剂量反应梯度效应）
for (y in lipid_bin) {
...
## 8.4.2 Box-Tidwell检验：连续自变量线性假设检验（以低HDL血症为例）
y_bin <- "low_hdl"
...
## 8.4.3 Hosmer-Lemeshow拟合优度检验（Logistic模型适配性）
for(y_bin in lipid_bin){
...
## 8.4.4 模型优化：仅纳入BMI二次项修正低HDL血症模型
...
### 8.5 敏感性分析（剔除极端值验证稳健性）
# 剔除膳食添加糖摄入量1%、99%分位数以外极端样本
...
## 8.6.3 性别分层Logistic回归（分男性、女性独立建模，剔除性别混杂变量）
...
# 线性模型vs样条模型拟合效果对比（AIC+ANOVA）
cat("\nHDL-C线性/样条模型AIC对比\n")
print(AIC(linear_hdl, ns_mod))
cat("\nHDL-C线性vs样条方差分析ANOVA\n")
anova(linear_hdl, ns_mod)

### 8.9 低HDL血症患病风险剂量-反应建模（二分类结局）
## 8.9.1 线性Logistic剂量反应模型
...
# 线性Logistic与GAM模型AIC对比
cat("\nlow_hdl线性Logistic/GAM模型AIC对比\n")
...
（4）统计与样本层面局限
    ...
    剂量反应分析仅覆盖现有样本摄入区间，对于极高添加糖摄入、极低摄入人群样本量偏少，曲 线两端置信区间宽，阈值拐点的参考价值受限；非线性分析中，HDL-C的样条非线性趋势无统计学意义，仅低HDL患病风险存在显著非线性，无法全面揭示四类血脂指标的阈值效应；
...
（1）学习 R 语言的困难
    ...
    正式开展统计分析后，我又遇到了更多难题。最开始做分层回归、循环统计时，我不会自定义函数、循环批量运算，四类血脂指标、三层校正模型只能逐段复制粘贴代码，不仅工作量巨大，还很容易出现变量名写错、参数设置不统一的问题。同时我对多元分层校正、Logistic 回归、样条非线性模型、GAM广义加性模型都很陌生，不清楚各类模型的适用场景、配套函数，也不知道完整分析代码该如何搭建。使用ggplot2绘图时，坐标轴、图例、标题、置信区间配色、分段调节等操作都不熟练，绘制出的箱线图、趋势散点图达不到学术期刊规范要求。
