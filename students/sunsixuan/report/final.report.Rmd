...
#我国老年人卫生服务利用影响因素及公平性分析

## 1.研究背景
### 1.1人口老龄化
《2024年度国家老龄事业发展公报》显示，截至2024年年末，全国60周岁及以上老年人口为31031万人，占总人口的22.0%；全国65周岁及以上老年人口为22023万人，占总人口的15.6%。我国已迈入深度老龄化社会，人口老龄化已成为我国重要的民生与公共卫生议题。我国老年人口对医疗、康复、长期照护等卫生服务的需求正日益增长。
### 1.2区域经济发展水平和人口老龄化程度不同步
研究数据显示2010—2020年，我国乡村人口老龄化增速快于城市，城乡老龄化差异正逐渐缩小。东北地区、黄河中下游地区成为老龄化率快速增长区，东北地区老年人口占比达到24.26%，而这些地区的经济发展水平却较低。
### 1.3卫生资源配置不均
大量优质卫生资源集中于三甲医院，社区卫生服务中心、乡镇卫生院等基层机构医护人员数量不足、专业能力薄弱。

## 2.研究问题
了解我国老年人卫生服务的实际利用现状；识别导致卫生服务利用差异的关键因素，分析各种因素的作用机制和影响效应；测算卫生服务利用的公平性水平。

## 3.数据来源与研究对象
利用北京大学中国健康与养老追踪调查（CHARLS）公开数据集。该数据库以我国45岁及以上中老年人为核心调查对象，系统收集人口学、社会经济、健康状况、卫生服务利用等多维信息，抽样覆盖全国28个省份，样本具有全国代表性。由于疫情限制，本研究拟选取2018年数据库中65岁及以上老年人为研究对象进行数据分析。
...
```
### 3.2保留2018年且年龄大于等于65岁的调查对象
```{r}
df_2018<-subset(charls,iwy==2018&age>=65)
```

## 4.变量说明
### 4.1统计研究对象患有的慢性病数量，将研究对象慢性病数量分为0种，1种，2种,3种,4种,5种及以上。
```{r}
chronic_vars <- c(
  "hibpe",   
  "diabe",    
...
```
### 4.2将研究对象年龄分为65-74岁，75-84岁，85岁及以上三组
```{r}
df_2018 <- df_2018 %>%
  mutate(
    age_group = case_when(
...
summary(fit_single)            # P值
```
#### 8.1.2受教育水平与门诊率、住院率的卡方检验和线性趋势检验
```{r,include=FALSE}
library(DescTools)
```
```{r}
...
```
#### 8.1.3家庭年人均消费与门诊率、住院率的卡方检验和线性趋势检验
```{r}
...
```
#### 8.1.4自评健康状况与门诊率、住院率的卡方检验与线性趋势检验
```{r}
...
```
#### 8.1.5慢性病数量与门诊率、住院率的卡方检验与线性趋势检验
```{r}
...
## 9.结果解释
### 9.1门诊率多因素回归
结果显示：在控制年龄、性别、文化程度、婚姻状况、城乡居住地、吸烟史、医疗保险等混杂变量后：慢性病患病数量每增加1个等级，老年人门诊就诊风险上升25.09%（OR=1.2509，95%CI：1.1909-1.3141，P＜0.001）；自评健康状况每提升1个等级，门诊就诊风险下降26.81%（OR=0.7319，95%CI：0.6738-0.7951，P＜0.001）；家庭消费水平越高，门诊就医概率越高（OR=1.1133，P=0.001）；其余人口学变量、医保与吸烟史对门诊就诊率无统计学影响（P均＞0.05）。

### 9.2住院率的多因素回归
结果显示：调整所有混杂因素后:年龄越高，住院概率显著上升；与65-74岁组相比，85岁及以上老年人住院风险升高108.81%（OR=2.0881，P=0.001）；女性住院风险高于男性（OR=1.3915，P＜0.001）；有医疗保险者住院风险显著更高（OR=3.4476，P＜0.001）；家庭消费水平越高，住院率越高（OR=1.1219,P<0.001）；慢性病数量每增加1级，住院就诊风险上升34.09%（OR=1.3409，95%CI：1.2806-1.4040，P＜0.001）；自评健康状况越好，住院概率越低（OR=0.6550，P＜0.001）；相比于抽烟者，无吸烟史的老年人住院率显著更低（OR=0.6287,P<0.001）；婚姻、城乡居住地对住院率无统计学意义（P＞0.05）。

### 9.3不同城乡居住地分层分析
门诊服务利用分层分析结果显示，慢性病负担与自评健康状况对城乡老年人门诊就医行为均存在显著影响。医疗保险仅在农村人群中显著提升门诊服务利用（OR=2.010，95%CI：1.151-3.798），在城镇人群中效应不显著。住院服务利用分层分析显示，慢性病负担、自评健康、高龄、医疗保险对城乡老年人住院就医均存在显著影响。研究结论表明，慢性病数量越多、自评健康状况越差城乡老年人门诊和住院两类就医概率均显著上升；医疗保险仅显著提升农村老年人门诊利用率，对城镇老年人无作用，但医保显著提升城乡老年人住院概率。
...
### 9.5公平性分析
以家庭人均消费作为社会经济分层指标，采用集中指数与集中曲线评价老年人门诊、住院卫生服务利用的经济公平性。结果显示，门诊服务原始集中指数CI=0.0945，Erreygers校正指数ECI=0.0782；住院服务原始集中指数CI=0.0935，校正指数ECI=0.1091，两类服务集中指数均为正值，且集中曲线均落在45°绝对公平对角线下方，提示老年人门诊、住院服务利用均存在向高收入群体倾斜的经济不公平特征。对比门诊与住院结果可见，住院服务校正集中指数更高，集中曲线偏离公平线距离更远，说明住院服务利用的经济不公平程度高于门诊服务，高收入老年人相较于低收入群体，住院服务利用优势更突出。

## 10.局限性
研究数据库选用2018年调查数据，为横断面研究且时效性有限，难以推断因果关系；公平性分析中仅采用集中指数进行测算，无法量化各指标对卫生服务利用不公平的贡献程度。

## 11.个人总结
本次研究基于CHARLS数据库，采用Logistic回归、分层分析与集中指数法，分析门诊、住院服务利用的影响因素与经济公平特征。实操中，我独立完成数据清洗、分层建模、公平指数计算与集中曲线绘制等，掌握了基础的R语言的代码编写与rmarkdown的报告输出，锻炼了我的实证输出能力。
