...
data_clean <- data_clean %>%
  mutate(
    ...
    # 如果明确用药，记为1
    # 如果明确不用药，记为0
    # 如果没有被医生告知高血压，BPQ040A缺失可能是跳题，暂时视为未用药
    ...
    # 主分析血压定义：140/90标准
    # 只要收缩压或舒张压任一达到标准，就认为现场测量提示高血压
    # 如果有血压测量，且都没有达到标准，就认为现场测量未提示高血压
    measured_hbp_140 = case_when(
      (!is.na(mean_sbp) & mean_sbp >= 140) |
        (!is.na(mean_dbp) & mean_dbp >= 90) ~ 1,
      
      (!is.na(mean_sbp) | !is.na(mean_dbp)) &
        (is.na(mean_sbp) | mean_sbp < 140) &
        (is.na(mean_dbp) | mean_dbp < 90) ~ 0,
    ...
    measured_hbp_130 = case_when(
      (!is.na(mean_sbp) & mean_sbp >= 130) |
        (!is.na(mean_dbp) & mean_dbp >= 80) ~ 1,
      
      (!is.na(mean_sbp) | !is.na(mean_dbp)) &
        (is.na(mean_sbp) | mean_sbp < 130) &
        (is.na(mean_dbp) | mean_dbp < 80) ~ 0,
    ...
    # 如果现场血压正常，即使降压药信息缺失，先暂时定义为非高血压
    hypertension_main = case_when(
      measured_hbp_140 == 1 | antihypertensive_med == 1 ~ 1,
      measured_hbp_140 == 0 &
        (antihypertensive_med == 0 | is.na(antihypertensive_med)) ~ 0,
      TRUE ~ NA_real_
    ),
    ...
    hypertension_extended = case_when(
      ...
      measured_hbp_140 == 0 &
        (antihypertensive_med == 0 | is.na(antihypertensive_med)) &
        (bpq020_clean == 0 | is.na(bpq020_clean)) ~ 0,
